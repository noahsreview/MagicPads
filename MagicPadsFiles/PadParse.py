...
import logging
import MagicPadsFiles.osc as osc
...
def parse_button_press(button_state, lp):
    ...
    if button_state:
        ...
        if(vel > 0):
            print(x, y, coords_to_number(x,y))
            if(x == 7 and y == 9):
                image_to_launchpad_colors("your_image.png")
                logging.warning("resetting image")
        ...
        if vel > 0:
            ...
            osc.sendOSC(coords_to_number(x, y), 1)
        else:
            send_led_message(lp, x, y, False)  # Restore original image color
            osc.sendOSC(coords_to_number(x, y), 0)
